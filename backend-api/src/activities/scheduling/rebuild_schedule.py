...
async def run_rebuild_schedule_reconciliation(client: Client) -> RebuildScheduleAction:
    spec = derive_rebuild_schedule_spec(
        enabled=settings.search_index_rebuild_schedule_enabled,
        cron=settings.search_index_rebuild_schedule_cron,
        timezone=settings.search_index_rebuild_schedule_timezone,
    )
