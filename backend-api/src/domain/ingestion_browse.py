...
class IngestionPage(BaseModel, frozen=True):
...
class IngestionBrowser:
    ...
    def list_attempts(
        ...
        *,
        limit: int,
        offset: int,
        view: IngestionView = IngestionView.ALL,
    ...
    def _view_predicate(
        ...
        *,
        ...
        if view == IngestionView.ALL:
            return None
        if view == IngestionView.FAILED:
            return IngestURL.status == ProcessingStatus.FAILED
        if view == IngestionView.COMPLETED:
    ...
    def _outcome_predicate(self, outcome: IngestOutcome) -> ColumnElement[bool]:
        if outcome == IngestOutcome.INGESTED:
            return _INGESTED
        if outcome == IngestOutcome.DEDUPED:
            return _DEDUPED
    ...
    def _derive_outcome(self, attempt: IngestURL) -> IngestOutcome:
        ...
        return IngestOutcome.IN_FLIGHT
