...
class TestImageUploadStager:
    ...
    async def test_stores_bytes_and_returns_artifact_reference(self) -> None:
        storage = MagicMock()
        ...
        assert staged.artifact_key == args[1]
    ...
    async def test_stage_uses_private_storage_without_creating_a_url(self) -> None:
        ...
        assert storage.uploaded == [(b"image-bytes", staged.artifact_key, "image/png")]
        assert staged.model_dump() == {
            "kind": "staged_upload",
