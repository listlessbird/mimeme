from __future__ import annotations

import pytest
from sqlalchemy import Engine, delete, func, select, text
from sqlalchemy.exc import TimeoutError as SQLAlchemyTimeoutError

from mimeme.config import DatabaseConfig
from mimeme.db import Db
from mimeme.db.schema import IndexBuild


def _db(engine: Engine, **overrides: object) -> Db:
    if engine.dialect.name != "postgresql":
        pytest.skip("async DB tests require PostgreSQL")
    return Db(
        DatabaseConfig(
            url=engine.url.render_as_string(hide_password=False),
            **overrides,
        )
    )


async def test_read_session_executes_database_work(db_engine: Engine) -> None:
    db = _db(db_engine)
    try:
        async with db.read_session() as session:
            result = await session.execute(text("select 1"))
        assert result.scalar_one() == 1
    finally:
        await db.close()


async def test_saturated_pool_raises_timeout_error(db_engine: Engine) -> None:
    db = _db(db_engine, pool_size=1, max_overflow=0, pool_timeout_s=0.1)
    try:
        async with db.read_session() as held:
            await held.execute(text("select 1"))
            with pytest.raises(SQLAlchemyTimeoutError):
                async with db.read_session() as starved:
                    await starved.execute(text("select 1"))
    finally:
        await db.close()


def _delete_build(db_engine: Engine, version: str) -> None:
    with db_engine.begin() as connection:
        connection.execute(delete(IndexBuild).where(IndexBuild.version == version))


def _build_count(db_engine: Engine, version: str) -> int:
    with db_engine.connect() as connection:
        count = connection.scalar(
            select(func.count()).select_from(IndexBuild).where(IndexBuild.version == version)
        )
        return count or 0


async def test_read_session_does_not_commit_database_work(db_engine: Engine) -> None:
    version = "test-async-read-session-no-commit"
    _delete_build(db_engine, version)
    db = _db(db_engine)
    try:
        async with db.read_session() as session:
            session.add(IndexBuild(version=version, is_active=False))
            await session.flush()
        assert _build_count(db_engine, version) == 0
    finally:
        await db.close()


async def test_write_session_commits_successful_database_work(db_engine: Engine) -> None:
    version = "test-async-write-session-commits"
    _delete_build(db_engine, version)
    db = _db(db_engine)
    try:
        async with db.write_session() as session:
            session.add(IndexBuild(version=version, is_active=False))
        assert _build_count(db_engine, version) == 1
    finally:
        await db.close()
        _delete_build(db_engine, version)


async def test_write_session_rolls_back_database_work_on_exception(db_engine: Engine) -> None:
    version = "test-async-write-session-rolls-back"
    _delete_build(db_engine, version)
    db = _db(db_engine)
    try:
        with pytest.raises(RuntimeError, match="boom"):
            async with db.write_session() as session:
                session.add(IndexBuild(version=version, is_active=False))
                await session.flush()
                raise RuntimeError("boom")
        assert _build_count(db_engine, version) == 0
    finally:
        await db.close()
