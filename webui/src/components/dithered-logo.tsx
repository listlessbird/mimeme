...
const toGrayscaleGrid = (
	...
	for (let y = 0; y < outH; y++) {
		for (let x = 0; x < outW; x++) {
			...
			let luma =
				blurAlpha > 0.01
					? (0.299 * pixels[idx]! + 0.587 * pixels[idx + 1]! + 0.114 * pixels[idx + 2]!) / blurAlpha
					: 0;
...
const buildRoundedMask = (w: number, h: number, radiusPct: number): Set<number> => {
...
export function DitheredLogo({
	...
	return (
		<div className={cn("relative h-60 w-60 text-black", className)} style={style}>
